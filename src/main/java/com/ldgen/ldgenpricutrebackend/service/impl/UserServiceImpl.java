...
import com.ldgen.ldgenpricutrebackend.model.enums.UserRoleEnum;
...
import com.ldgen.ldgenpricutrebackend.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        //3,密码加密
        String encryptPassword = getEncryptPassword(userPassword);  //这里使用加密算法
        //4.插入到数据库
        User user = new User();
        user.setUserAccount(userAccount);
        user.setUserPassword(encryptPassword); //使用加密后密码
        user.setUserName("无名");
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        //不存在抛异常
        if (user == null) {
            log.info("用户登录失败，用户或者密码错误");
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户不存在或者是密码错误");
        }
        //4.记录用户的登录态
        request.getSession().setAttribute(USER_LOGIN_STATE, user);
        return this.getLogUserVO(user);
     ...
     * @return 脱敏后的用户信息
     */
    private LoginUserVO getLogUserVO(User user) {
        LoginUserVO loginUserVO = new LoginUserVO();
        BeanUtils.copyProperties(user, loginUserVO);
        return loginUserVO;
    }
